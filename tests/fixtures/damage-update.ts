...
export const damageUpdate: EntityUpdateFunction<Components> = (world, entityId, components, queries, callbacks) => {
	const health = components.health;
	if(!health) {
		return;
	}

	const damage = typeof world.damage === 'number' ? world.damage : 1;
	health[0] -= damage;
